...
namespace ModbusSimulator.ViewModels
{
    public class UcSlaveViewModel : BindableBase, IDialogAware
    {
        ...
        public DelegateCommand<Exception> DisconnectCMD => new DelegateCommand<Exception>(OnDisconnect);
        public DelegateCommand ExecuteCMD => new DelegateCommand(async () => await OnExecute());

        public UcSlaveViewModel()
        {
        ...
        private void OnConnect()
        {
            ...
            try
            {
                ...
                _slave.SetEndian(SelectedEndian.Value);
        ...
        private void OnDisconnect(Exception e = null)
        {
        ...
        private async Task OnExecute()
        {
            ...
            try
            {
                if (SelectedMode == "读")
                {
                    var values = await _slave.Read_Multi_Value_FromHoldingRegisters(StartAddress.Value, SelectedType.Value, NumRegisters.Value);
                ...
                else if (SelectedMode == "写")
                {
                    ...
                    await _slave.Write_Multi_Value_ToHoldingRegisters(StartAddress.Value, SelectedType.Value, values);
                }
            }
            catch (Exception e)
            {
        ...
        private bool CanExecute()
        {
            ...
            if (SelectedMode == null || StartAddress == null || SelectedType == null || NumRegisters == null)
            {
                var info = new MessageBoxInfo
                {
                    Message = "请填写参数",
